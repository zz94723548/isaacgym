...
def load_ACT(model_path, args_file:str = None, override_args:dict = None) -> ACTPolicy:
    """
    ...
    if args['backbone'] == "clip_backbone":
        ...
        if args['gelsight_backbone_path'] != 'none' and args['vision_backbone_path'] != 'none':
            vision_model.load_state_dict(torch.load(args['vision_backbone_path'], map_location='cpu'))
            gelsight_model.load_state_dict(torch.load(args['gelsight_backbone_path'], map_location='cpu'))
    ...
    act = ACTPolicy(state_dim = args['state_dim'],
                    ...
                    kl_weight = args.get('kl_weight', 10.0),
    ...
    act.load_state_dict(torch.load(model_path, map_location='cpu'))
